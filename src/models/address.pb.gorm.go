...
type AddressORM struct {
	CurrentBalanceDec float64 `gorm:"index:address_idx_current_balance_dec"`
	CurrentBalanceHex string
...
func (m *Address) ToORM(ctx context.Context) (AddressORM, error) {
	...
	to.CurrentBalanceHex = m.CurrentBalanceHex
	to.CurrentBalanceDec = m.CurrentBalanceDec
...
func DefaultApplyFieldMaskAddress(ctx context.Context, patchee *Address, patcher *Address, updateMask *field_mask1.FieldMask, prefix string, db *gorm1.DB) (*Address, error) {
	...
	for _, f := range updateMask.Paths {
		...
		if f == prefix+"CurrentBalanceHex" {
			patchee.CurrentBalanceHex = patcher.CurrentBalanceHex
			continue
		}
		if f == prefix+"CurrentBalanceDec" {
			patchee.CurrentBalanceDec = patcher.CurrentBalanceDec
			continue
		}
	}
	if err != nil {
