...
func TestAddressesEndpointDetailTokens(t *testing.T) {
	assert := assert.New(t)
	...
	// Test number
	resp, err = http.Get(addressesServiceURL + addressesServiceRestPrefx + "/addresses/address-tokens/" + addressPublicKey)
	assert.Equal(nil, err)
